...
public class ActionImpl {
  ...
  public int pc;
  private int waitCnt;
  private float aimSpeed, moveSpeed;
  private int mvspCnt;
  ...
  public void setMoveStatus(int mvdrCnt, float mvDrc, boolean isAim,
   int mvspCnt, float mvSpeed, int acclCnt, float mvMx, float mvMy) {
    ...
    this.mvspCnt = mvspCnt; this.moveSpeed = mvSpeed;
  ...
  public void move() {
      ...
      if ( repeat <= 0 ) {
        ...
        if ( parent != null ) {
          parent.setMoveStatus(
           mvdrCnt, mvDrc, isAim, mvspCnt, moveSpeed, acclCnt, mvMx, mvMy);
    ...
    if ( ac instanceof Repeat ) {
      ...
      if ( type != null && type.equals(SEQUENCE_KEYWORD) ) {
        moveSpeed = BulletmlUtil.getFloatValue(s.getContent(), prms);
        ...
        moveSpeed = (aimSpeed - bullet.speed) / mvspCnt;
      }
